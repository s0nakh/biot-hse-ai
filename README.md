...
[![Статус](https://img.shields.io/badge/статус-MVP%20v1.0-ff6b35?style=flat-square)](.)
[![Стек](https://img.shields.io/badge/стек-Node.js%20%7C%20React%20%7C%20Chart.js-4f8cff?style=flat-square)](.)
[![Данные](https://img.shields.io/badge/данные-520%20инцидентов%20%7C%201050%20наблюдений-00e096?style=flat-square)](.)
...
Демо-версия MVP доступна по ссылке 👉🏻 [нажми меня](https://biot-hse-ai.netlify.app/) 
...
Система одновременно анализирует 8 факторов риска и рассчитывает взвешенный индекс:

```
Тяжесть инцидента                → 10–65 баллов
Ночная смена                     → +8
Погодные условия                 → +5..+12
Нарушение требований СИЗ¹⁰       → +12
Малый опыт работника (<5 лет)    → +5..+10
Опасные ситуации (near miss⁷) за последние 30 дней → +5..+12
Просроченное обучение (>90 дней) → +10
Основная причина высокого риска   → +8
(high-risk root cause)
─────────────────────────────────────────────────────
Индекс риска: 0–100
Критический ≥75 | Высокий ≥50 | Средний ≥25 | Низкий <25
```
...
- Документ доступен для редактирования и отправки
...
Синтетические данные, сгенерированные по образцу реальных HSE³-данных (ТЗ раздел 4.1):

**incidents.csv — 520 записей (2022–2025)**
```
...
**korgau_cards.csv — 1050 записей (2022–2025)**
```
...
```
biot-hse-ai/
├── biot-preview.html          ← Standalone демо (открыть в браузере)
├── README.md
├── render.yaml
├── backend/
│   └── src/
│       ├── data/generateDataset.js    ← Генератор синтетических данных
...
**Q: Данные синтетические — как можно доверять выводам системы?**  
A: ТЗ раздел 4.1 прямо предусматривает использование синтетических данных для MVP. Синтетика сгенерирована по реальным схемам данных HSE-систем нефтегазовой отрасли. Все алгоритмы (scoring, прогноз, корреляция, алерты) работают идентично на реальных данных — нужно лишь подключить ИСОТ⁵ КМГ-Кумколь через API.
...
**София Ахметова** — Специалист по QHSE, 9 лет в нефтегазовой отрасли (Baker Oil Tools, Weatherford, Halliburton)   
...
[![Status](https://img.shields.io/badge/status-MVP%20v1.0-ff6b35?style=flat-square)](.)
[![Stack](https://img.shields.io/badge/stack-Node.js%20%7C%20React%20%7C%20Chart.js-4f8cff?style=flat-square)](.)
[![Data](https://img.shields.io/badge/data-520%20incidents%20%7C%201050%20observations-00e096?style=flat-square)](.)
...
Demo version of MVP is available here 👉🏻 [click me](https://biot-hse-ai.netlify.app/) 
...
Analyzes 8 risk factors simultaneously, producing a weighted index:

```
Severity                   → 10–65 pts
Night shift                → +8
Weather conditions         → +5..+12
PPE¹⁰ non-compliance       → +12
Low experience (<5 yrs)    → +5..+10
Near miss⁷ (last 30 days)  → +5..+12
Training overdue (>90 days) → +10
High-risk root cause        → +8
──────────────────────────────────────
Risk Score: 0–100
...
**Q: The data is synthetic — how reliable are the system's outputs?**  
A: TS Section 4.1 explicitly allows synthetic data for MVP. The dataset is generated according to real HSE data schemas from the oil & gas industry. All algorithms (scoring, forecasting, correlation, alerts) work identically on real data — the only step needed is connecting KMG-Kumkol's HSEMS⁵ via API.
...
**Sophia Akhmetova** — QHSE Specialist, 9 years in oil & gas (Baker Oil Tools, Weatherford, Halliburton)  
