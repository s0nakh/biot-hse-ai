...
[![Статус](https://img.shields.io/badge/статус-MVP%20v3.0-ff6b35?style=flat-square)](.)
...
- [Функционал — 9 страниц](#-функционал--9-страниц)
- [Экономический эффект](#-экономический-эффект)
- [Технологии](#-технологии)
- [Быстрый старт](#-быстрый-старт)
- [Словарь терминов](#-словарь-терминов)
- [🚀 Обновления после Хакатона](#-обновления-после-хакатона)
- [👤 Автор](#-автор)
...
| 9 | **Новый отчёт** | Форма ввода инцидента/наблюдения Коргау, Live Risk Score, мгновенное добавление в аналитику |
...
| 5 | Telegram Alerts — уведомления через `t.me/` deeplink (живёт в браузере) | ✅ Реализовано в v3.0 |
...
## 🚀 Обновления после Хакатона

Проект продолжает активно развиваться. Ниже представлены ключевые доработки, внедренные после завершения хакатона:

### v3.0 — Интеллектуальный RCA и глубокая кастомизация (Апрель 2026)
* ✅ Контекстный RCA:** Алгоритм анализа первопричин теперь учитывает специфику подразделения и тип инцидента.
    * *Разведка и Добыча:* Фокус на буровых операциях, системах LOTO и управлении вахтами.
    * *Транспортировка:* Анализ ПДД, техсостояния ТС и дорожных условий.
    * *Переработка:* Контроль огневых работ и исправности вентиляционных систем.<br>
**Уникальные «5 Почему»:** Дерево причин больше не является статичным — для каждой комбинации «Департамент + Тип происшествия» генерируется логически верная цепочка аргументации.
* ✅ Умный NCR-генератор:** Автоматическое заполнение корректирующих действий (Corrective Actions) теперь полностью кастомизировано под конкретные риски подразделения.
* ✅ Тепловая карта инцидентов (вкладка Аналитика)**

&emsp;&emsp;**Что показывает:** матрица 7 строк (дни недели Пн–Вс) × 24 столбца (часы 00–23). Цвет ячейки = количество инцидентов в это время.

&emsp;&emsp;**Данные:** 220 реальных инцидентов КМГ-Кумколь с полями `hr` (час) и `dow` (день недели), извлечёнными из колонки «Время возникновения происшествия» в Excel.

&emsp;&emsp;**Цветовая шкала:**<br>
&emsp;&emsp;🔵 Синий (rgba 79,140,255) = 1–2 инцидента<br>
&emsp;&emsp;🟡 Жёлтый (rgba 255,205,60) = 3–5<br>
&emsp;&emsp;🔴 Красный (rgba 255,69,96) = 6–8

&emsp;&emsp;**Топ пики (реальные данные КМГ):**<br>
&emsp;&emsp;1. Среда 15:00 — 8 инцидентов<br>
&emsp;&emsp;2. Понедельник 15:00 — 8 инцидентов<br>
&emsp;&emsp;3. Среда 14:00 — 7 инцидентов

&emsp;&emsp;**Инсайт:** пик рисков — среда и пятница 14–16ч. Середина рабочей недели, накопленная усталость.

&emsp;&emsp;**На мобильном:** тап на ячейку → информационная панель снизу с деталями: день/час, количество инцидентов, % от общего, типы инцидентов, подразделения, рекомендация.

* ✅ Загрузка своего Excel (панель под шапкой)**

&emsp;&emsp;**Что делает:** клиент может загрузить свой файл инцидентов (.xlsx/.xls) и BiOT мгновенно пересчитывает ВСЕ графики, таблицы и heatmap на его данных. Без сервера — всё в браузере.

&emsp;&emsp;**Как работает:**<br>
&emsp;&emsp;1. Клиент нажимает «📤 Загрузить Excel» → выбирает файл<br>
&emsp;&emsp;2. SheetJS читает файл прямо в браузере (без отправки на сервер)<br>
&emsp;&emsp;3. Алгоритм автоматически определяет колонки: дата, тип, подразделение, локация, время<br>
&emsp;&emsp;4. Строит новый DS-массив с Risk Score для каждой записи<br>
&emsp;&emsp;5. Пересчитывает heatmap, графики, KPI, RCA, CPAR — всё обновляется

&emsp;&emsp;**Умный маппинг колонок:** ищет по ключевым словам на русском и английском. Работает с любым форматом Excel где есть хотя бы колонка с датой.

&emsp;&emsp;**Индикаторы состояния:**
- Бейдж «HACKATHON MVP» → меняется на «ВАШИ ДАННЫЕ» (синий)
- Полоса под шапкой показывает имя файла и количество распознанных записей
- Кнопка «↩ Данные КМГ» — возврат к исходным данным КМГ-Кумколь одним кликом

&emsp;&emsp;**Технология:** SheetJS (xlsx.js) — открытый исходный код.

* ✅ Telegram-алерты (вкладка Коргау & Алерты))**

&emsp;&emsp;**Что делает:** при **Critical** или **High** алерте появляется кнопка «📨 Telegram». При нажатии — открывается Telegram с готовым сообщением адресованным ответственному HSE-менеджеру/инспектору.

&emsp;&emsp;**Как настроить:** первый раз нажимаете «Настроить Telegram» → вводите @никнейм получателя → Сохранить. Никнейм сохраняется в localStorage браузера.

**Формат сообщения:**
```
⚠️ BiOT HSE AI — АЛЕРТ
Уровень: 🔴 КРИТИЧЕСКИЙ
Объект: Drilling · Категория: LOTO
Нарушения LOTO: 7 раз за 30 дней. Требуется остановка работ.
📊 Подробности: https://biot-hse-ai.netlify.app/
Требуется немедленное реагирование.
```

&emsp;&emsp;**Техническая суть:** использует `t.me/@username?text=...` — открывает Telegram с предзаполненным текстом. Никакого сервера, никакого API ключа. Работает на любом устройстве где установлен Telegram.

&emsp;&emsp;**Пульсация:** **Critical алерты** мигают — нельзя пропустить.

* ✅ Вкладка 9 — НОВЫЙ ОТЧЁТ (New Report)**

&emsp;&emsp;**Назначение:** HSE-инженер заполняет инцидент или наблюдение Коргау прямо в BiOT вместо Excel. Запись мгновенно добавляется в аналитику.

&emsp;&emsp;**3 типа записи:**
- 🚨 **Инцидент/НС** — несчастный случай, микротравма, ДТП, пожар
- 👁 **Наблюдение Коргау** — нарушение которое увидел на объекте
- ✅ **Хорошая практика** — позитивное наблюдение BBS

&emsp;&emsp;**Обязательные поля:** дата, подразделение, место происшествия.

&emsp;&emsp;**Дополнительные поля для инцидентов:** тип, тяжесть, корневая причина, ответственный, описание, принятые меры.

&emsp;&emsp;**Факторы риска (чекбоксы):**
- 🌙 Ночная смена (+8 к Risk Score)
- 🦺 Нарушение СИЗ (+12)
- ⚡ Near Miss за 30 дней (+10)
- 📋 Просрочка инструктажа (+10)
- ☔️ Погодные условия (+5..+8)

&emsp;&emsp;**Live Risk Score:** пока заполняется форма — Risk Score пересчитывается в реальном времени. Виден цвет уровня (Low/Medium/High/Critical) и список факторов которые повлияли.

&emsp;&emsp;**После сохранения:**
- Инцидент → добавляется в DS, обновляет все графики и heatmap
- Наблюдение Коргау → добавляется в KG_RAW, обновляет вкладку Коргау
- Показывается сообщение «✅ Запись добавлена»

&emsp;&emsp;**Ответ на вопрос «данные сохраняются между сессиями?»:** В текущей версии данные живут в памяти браузера до обновления страницы. В production-версии подключается PostgreSQL — данные сохраняются навсегда.
...
[![Status](https://img.shields.io/badge/status-MVP%20v3.0-ff6b35?style=flat-square)](.)
...
| 9 | **New Report** | Incident/Korgau observation form, Live Risk Score, instant analytics update |
...
The project is under active development. Below are the key enhancements implemented following the completion of the hackathon:

### v3.0 — Intelligent RCA & Deep Customization (April 2026)

* **✅ Context-Aware RCA:** The Root Cause Analysis engine now distinguishes between specific departments and incident types.
    * *Exploration & Production (E&P):* Focus on drilling operations, LOTO systems, and crew shift management.
    * *Transportation:* Analysis of traffic regulations, vehicle technical conditions, and road safety.
    * *Refining:* Monitoring of hot works and ventilation system integrity.
* **Dynamic "5 Whys":** The cause tree is no longer static. It generates a logically unique reasoning chain for every combination of "Department + Incident Type."
* **✅ Smart NCR Generator:** Auto-filling of Corrective Actions is now fully customized based on specific departmental risks and incident categories.
* **✅ Incident Heatmap (Analytics Tab):**<br> **Visualization:** A 7×24 matrix (Days: Mon–Sun × Hours: 00–23). Cell color intensity represents the incident frequency during that specific time slot.

&emsp;&emsp;**Data Source:** Based on 220 real-world incidents from the KMG-Kumkol dataset. Time-series data (hr for hour and dow for day of week) was extracted directly from the "Incident Occurred Time" column in Excel.

&emsp;&emsp;**Color Scale (Risk Intensity):**

&emsp;&emsp;🔵 Blue (rgba 79,140,255) = 1–2 incidents<br>
&emsp;&emsp;🟡 Yellow (rgba 255,205,60) = 3–5 incidents<br>
&emsp;&emsp;🔴 Red (rgba 255,69,96) = 6–8 incidents

&emsp;&emsp;**Key Peak Areas (Real KMG Data):**<br>
&emsp;&emsp;Wednesday 15:00 — 8 incidents<br>
&emsp;&emsp;Monday 15:00 — 8 incidents<br>
&emsp;&emsp;Wednesday 14:00 — 7 incidents

&emsp;&emsp;**Actionable Insight:** Risk peaks occur predominantly on Wednesdays and Fridays between 14:00 and 16:00. This suggests a correlation with mid-week fatigue and accumulated stress before the weekend.

&emsp;&emsp;**Mobile Experience:** Optimized for touch. Tap any cell to trigger a persistent Information Panel at the bottom, providing: day/hour, incident count, % of total, incident types, involved departments, and specific safety recommendations.

* **✅ Custom Excel Upload (Header Panel)**<br>

&emsp;&emsp; **Feature:** Clients can upload their own incident log files (.xlsx/.xls), and BiOT instantly recalculates ALL charts, tables, and the heatmap using their data. Zero-server processing — everything happens locally in the browser.

&emsp;&emsp; **Workflow:**

&emsp;&emsp; User clicks "📤 Upload Excel" → selects a file. SheetJS parses the file directly in the browser (no data is sent to a server). The algorithm automatically maps columns: Date, Type, Department, Location, and Time. Generates a new Data Science array with a Risk Score for every record. Recalculates the Heatmap, KPIs, RCA, and CPAR modules in real-time.

&emsp;&emsp;**Smart Column Mapping:** Uses keyword-based mapping in both English and Russian. Compatible with any Excel format containing at least a "Date" column.

&emsp;&emsp;**Status Indicators:**

&emsp;&emsp;"HACKATHON MVP" badge → switches to "YOUR DATA" (Blue).

&emsp;&emsp;Header info-bar displays the filename and the number of recognized records.

&emsp;&emsp;"↩ KMG Data" button — one-click revert to the original KMG-Kumkol dataset.

&emsp;&emsp;**Technology:** SheetJS (xlsx.js) — Open-source and lightweight.
Privacy Note: "Does data leave the device?" No. The file is read by JavaScript locally. No data is transmitted to external servers.

* **✅ Telegram Alerts (Korgau & Alerts Tab)**<br>

&emsp;&emsp;**Feature:** When a **"Critical"** or **"High"** alert is triggered, a "📨 Telegram" button appears. Clicking it opens Telegram with a pre-formatted message addressed to the responsible HSE Manager/Inspector.

&emsp;&emsp;**Setup:** Click "Setup Telegram" → enter the recipient's @username → Save. The username is stored securely in the browser's localStorage.

&emsp;&emsp;**Message Format:**

```
⚠️ BiOT HSE AI — ALERT
Level: 🔴 CRITICAL
Facility: Drilling · Category: LOTO
LOTO Violations: 7 times in 30 days. Work stoppage required.
📊 Details: https://biot-hse-ai.netlify.app/
Immediate response required.
```
Technical implementation: uses `t.me/@username?text=...` deep-linking. No server-side API or bot tokens required. Works on any device with Telegram installed.

&emsp;&emsp;**Visual Cue:** Critical alerts used to ensure they are never overlooked.

* **✅ Tab 9 — NEW REPORT**<br>

&emsp;&emsp;**Purpose:** Allows HSE Engineers to log incidents or Korgau observations directly within BiOT instead of Excel. Entries are instantly integrated into the analytics engine.

&emsp;&emsp;**3 Entry Types:**<br>

&emsp;&emsp;🚨 Incident/Accident — Injuries, First Aid, MVA, Fire.<br>
&emsp;&emsp;👁 Korgau Observation — Safety violations observed on-site.<br>
&emsp;&emsp;✅ Good Practice — Positive BBS (Behavior-Based Safety) observations.

&emsp;&emsp;**Mandatory Fields:** Date, Department, and Location.
&emsp;&emsp;**Incident Details:** Type, Severity, Root Cause, Responsible Party, Description, and Actions Taken.

&emsp;&emsp;**Risk Factors (Checkboxes):**<br>
&emsp;&emsp;🌙 Night Shift (+8 Risk Score)<br>
&emsp;&emsp;🦺 PPE Violation (+12)<br>
&emsp;&emsp;⚡ Near Miss within 30 days (+10)<br>
&emsp;&emsp;📋 Overdue Training/Briefing (+10)<br>
&emsp;&emsp;☔️ Weather Conditions (+5 to +8)

&emsp;&emsp;**Live Risk Scoring:** As the form is filled, the Risk Score is recalculated in real-time. Users can see the risk level color (Low/Medium/High/Critical) and the specific factors driving the score.

&emsp;&emsp;**Post-Submission:** Incidents update the DS array, refreshing all charts and the heatmap. Korgau Observations update the KG_RAW array and the Korgau tab.

&emsp;&emsp;**Persistence Note:** In the current MVP, data persists in the browser memory during the session. The production version integrates PostgreSQL for permanent data storage.
